---
title: Releasing Beta and RC Versions
date: June 7, 2017
author: 
categories: 
url: https://make.wordpress.org/core/handbook/about/release-cycle/releasing-beta-versions/
---
...
*   For Beta 1 releases, it is strongly recommended to coordinate with the Outreach folks in #outreach to publish and promote a “Help Test WordPress X.Y” post (e.g., [6.1 post](https://make.wordpress.org/test/2022/09/21/help-test-wordpress-6-1/), [6.1 Slack message](https://wordpress.slack.com/archives/C015GUFFC00/p1663781207569069)).
...
*   Before starting, it is recommended that at least an hour is given for a code freeze as that gives a final chance for concerns to be raised and to give folks working a quick break before starting a release party.
*   Before starting, run all unit tests locally. This ensures that automated actions such as unit tests have a chance to finish before starting the release party.
...
*   Remind those attending the release party to not share links to the release package until after it’s been tested.
    *   Example ([archive](https://wordpress.slack.com/archives/C02RQBWTW/p1584479009394600)): **:siren: Reminder: Please do not share links to the package publicly until the all-clear is given after testing and the announcement post is published on WordPress.org. :siren:**
...
*   Verify latest GitHub Action checks are passing (i.e., all show ✅).
    *   Check the latest commit on [https://github.com/WordPress/wordpress-develop/actions?query=event%3Apush+branch%3Atrunk](https://github.com/WordPress/wordpress-develop/actions?query=event%3Apush+branch%3Atrunk)
...
*   Bump Version.
    *   Update the `$wp_version` in `trunk/src/wp-includes/version.php`  
        
        [![](https://make.wordpress.org/core/files/2017/06/wp-core-release-first-version-bump.png)](https://make.wordpress.org/core/files/2017/06/wp-core-release-first-version-bump.png)
        
        *   `$wp_version = '5.8-beta1-src';` for beta releases
        *   `$wp_version = '5.8.1-RC1-src';` for release candidates.
    ...
    *   Update the `version` in `composer.json` if it has not been updated yet. Like `package.json`, the version should not contain any beta or RC suffix.
    *   Commit to SVN.
        *   Example commit message: WordPress 6.9 Beta 1
    *   Ensure the version bump appears on [https://build.trac.wordpress.org/](https://build.trac.wordpress.org/). _This only needs to be verified by the person releasing via Mission Control_.
*   Build the packages.
    *   The release package needs to be built in [Mission Control](https://mc.wordpress.org/release/). Mission Control will need to be loaded/refreshed after the build appears in the step above.
*   *   Once it’s packaged, it needs to be tested well, including manually testing updates. (How do you do that? Checkout the [docs](https://make.wordpress.org/core/handbook/about/release-cycle/releasing-minor-versions/#testing-packages).)
    
    *   Enter build name?
    *   Click button?
*   Test the packages.
    
    *   Ask people to [test](https://make.wordpress.org/core/handbook/about/release-cycle/releasing-minor-versions/#testing-packages) by sharing the URL (e.g., [https://wordpress.org/wordpress-4.9-beta2.zip](https://wordpress.org/wordpress-4.9-beta2.zip)) and the Call for Testing post that highlights specific items to test ([6.8 example](https://make.wordpress.org/test/2025/03/04/help-test-wordpress-6-8/)). There are three ways to help test the package:
        *   Install and activate the [WordPress Beta Tester](https://wordpress.org/plugins/wordpress-beta-tester/) plugin
            
            *   For major RC releases, select the `Bleeding edge` channel and then `Beta/RC Only` stream.For a minor RC release, select the Point Release channel and the Nightlies stream (note that it is no longer possible to use beta tester to test the minor Beta/RC packages until after the nightlies have been built).
            
            Use WP-CLI to test: `wp core update https://wordpress.org/wordpress-4.9-beta2.zip`Directly download the Beta/RC version (e.g., https://wordpress.org/wordpress-5.8-RC4.zip)
...
*   Post another `/here` message to let all attendees know that the release will be proceeding and that no more testing results should be posted unless a major issue is discovered.
*   Another version bump.
    *   Commit: Post WordPress 4.8 Beta 2 version bump.

[![](https://make.wordpress.org/core/files/2017/06/wp-core-release-second-version-bump-1.png)](https://make.wordpress.org/core/files/2017/06/wp-core-release-second-version-bump-1.png)
...
*   Publish the announcement post (and the subsequent “Help Test WordPress X.Y” post if this is the Beta 1 release and a post was created).
    ...
    *   Share the post content in the #polyglots channel to help them generate translated versions (e.g., [Slack message for 6.1 Beta 1](https://wordpress.slack.com/archives/C02RP50LK/p1663780990205979) sharing the `code editor` content).
*   Announce in #core that release is available.
    *   Example: /here WordPress 5.8 Beta 1 is now available. Please help test! There’s a _lot_ to test this release.
...
*   Write a message in the [#props](https://wordpress.slack.com/messages/C0FRG66LR) channel thanking and giving props to everyone who tested or otherwise helped with the beta release process
    ...
    *   Include yourself! If you emceed the party, self-props is preferred over having to wait for someone else to come behind you and add your props. Yes, it’s awkward, but you earned it!
...
*   For first Beta in the release cycle:
    *   Add a new Milestone to Core Trac for the next release version. For example, when releasing WordPress `5.0`, add a Milestone for `5.1`. _You may need to ask someone with admin rights on Trac to do this._
...
[![](https://make.wordpress.org/core/files/2017/06/wp-core-release-second-version-bump.png)](https://make.wordpress.org/core/files/2017/06/wp-core-release-second-version-bump.png)