...
author: 
categories: 
url: https://make.wordpress.org/core/handbook/testing/automated-testing/phpunit/
---
...
Copy `wp-tests-config-sample.php` to `wp-tests-config.php`, and enter your database credentials. _Use a separate database._

### [Test running workflow options](#test-running-workflow-options)
...
#### [Workflow 1: Setting up the Docker container environment](#workflow-1-setting-up-the-docker-container-environment)
...
#### [Workflow 2: Setting up the Composer environment](#workflow-2-setting-up-the-composer-environment)
...
#### [Workflow 3: Setting up to run with the PHPUnit PHAR file with Composer available](#workflow-3-setting-up-to-run-with-the-phpunit-phar-file-with-composer-available)
...
#### [Workflow 4: Setting up to run with the PHPUnit PHAR file without Composer](#workflow-4-setting-up-to-run-with-the-phpunit-phar-file-without-composer)
...
To run PHPUnit tests _and_ all other watch tasks, use:

1

`npm run grunt` `watch` `-- --phpunit --group={testgroup}`

To run _only_ the PHPUnit watch task, use:
