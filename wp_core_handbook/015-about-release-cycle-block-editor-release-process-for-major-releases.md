---
title: Block Editor Release Process for Major Releases
date: October 16, 2020
author: 
categories: 
url: https://make.wordpress.org/core/handbook/about/release-cycle/block-editor-release-process-for-major-releases/
---
...
[Release team roles](#release-team-roles)
-----------------------------------------
...
[Planning before the first Major Release Beta](#planning-before-the-first-major-release-beta)
---------------------------------------------------------------------------------------------
...
### [Scheduling the last editor release and communicating deadlines](#scheduling-the-last-editor-release-and-communicating-deadlines)
...
### [The Roadmap post](#the-roadmap-post)
...
### [Setting up the project board](#setting-up-the-project-board)
...
### [Keeping the project board updated](#keeping-the-project-board-updated)
...
### [Running asynchronous triage sessions](#running-asynchronous-triage-sessions)
...
### [Assigning tasks from the board](#assigning-tasks-from-the-board)
...
[Knowing which features to include in the release](#knowing-which-features-to-include-in-the-release)
-----------------------------------------------------------------------------------------------------
...
### [Deciding on additional features to include](#deciding-on-additional-features-to-include)
...
### [How to handle removing features](#how-to-handle-removing-features)
...
[Experimental API management](#experimental-api-management)
-----------------------------------------------------------
...
### [Updating the Private API consent string](#updating-the-private-api-consent-string)
...
[Planning and writing dev notes](#planning-and-writing-dev-notes)
-----------------------------------------------------------------

_Check out_ [_this handbook page_](https://make.wordpress.org/core/handbook/tutorials/writing-developer-notes/) _for more context on dev note best practices._
...
### [Getting a list of commits](#getting-a-list-of-commits)
...
[General triage management](#general-triage-management)
-------------------------------------------------------

### [Routine triage](#routine-triage)
...
### [Release-specific triage](#release-specific-triage)
...
### [Determining how critical a bug is](#determining-how-critical-a-bug-is)
...
[Managing the first WordPress Beta release](#managing-the-first-wordpress-beta-release)
---------------------------------------------------------------------------------------
...
[Managing weekly Beta and RC releases](#managing-weekly-beta-and-rc-releases)
-----------------------------------------------------------------------------
...
### [Syncing Gutenberg code to WordPress Core](#cherry-picking-prs-for-release)
...
*   Check for PRs that contain [PHP changes that might require a _manual_ backport](https://github.com/WordPress/gutenberg/blob/trunk/docs/contributors/code/back-merging-to-wp-core.md). If any are required, either raise the backport PR in the WP Core repo or prompt the contributor to do so.
...
#### [Cherry Picking PRs](#cherry-picking-prs)
...
Please note that the automation can fail when there are conflicts, so you must still check the “Backport to WP x.x Beta/RC” label for closed PRs. Normally an automated comment is left to encourge the PR author to create a manual cherry-pick PR (with wp/x.x as the base). After merging the manual cherry-pick, you should remove the “Backport to WP x.x Beta/RC” label and add the “Backported to WP Core” label to the _original_ PR.

Pre-automation instructions, in case it completely failed.

Only follow these steps if automation _completely_ fails for _all_ PRs. Do not use these steps for conflict failures!
...
*   Create _another_ branch (the “cherry picking” branch) _based on_ the `wp/x.x` branch.
...
*   After manually solving a conflict, return to the original PR and
    ...
    *   _add_ the `Backported to WP Core` label.
...
*   Before merging, always [double-check that _all_ the necessary PRs have been identified and cherry picked](https://github.com/WordPress/gutenberg/issues?q=is%3Aissue+is%3Aclosed+label%3A%22Backport+to+WP+Beta%2FRC%22) as the automation is prone to occasional errors.
...
#### [Package updates and Core patches](#package-updates-and-core-patches)
...
*   **Note**: changes to `script-loader-packages.min.php` should only be expected _if_ there are **JavaScript** changes in the packages update. If there are no JavaScript changes (for example when changes are purely CSS) then the hashes remain the same because the compiled contents are identical.
