---
title: Releasing Minor Versions
date: July 24, 2015
author: 
categories: 
url: https://make.wordpress.org/core/handbook/about/release-cycle/releasing-minor-versions/
---

So, you wanna ship a minor version of WordPress? Okay, maybe you don’t _want_ to. Perhaps you _need_ to ship a minor version of WordPress. A minor release is intended for bugfixes and enhancements that do not add new deployed files and are at the discretion of the release lead with suggestions/input from component maintainers and committers. There’s a lot involved with shipping these releases, as documented below. If you’ve been through the release process before and see something missing _please add it_.
...
**Security patches should be created well ahead of time**. Different patches may be necessary for trunk and stable branches. Currently, we make an effort to back port patches to [all versions of WordPress since 4.7](https://wordpress.org/news/2025/06/dropping-security-updates-for-wordpress-versions-4-1-through-4-6/). However, back porting patches is not always possible and those versions of WordPress are _not officially supported_ as a result.
...
On release day, if a test fails or a security fix is no longer working properly, _punt_. Do not attempt last minute fixes. Either reschedule the release or do not include the fix in the release. Last minute fixes can lead to major bugs.
...
3.  Ensure that all commits have synced to [https://build.trac.wordpress.org/](https://build.trac.wordpress.org/). If not, find someone that can raise a flag with the systems team.
    *   _This is an important step that must not be skipped._
...
### [Selecting About Page Strings](#selecting-strings)
...
### [Testing Package Builds](#testing-packages)
